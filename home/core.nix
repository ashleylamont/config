...

{
    ...
    programs.atuin = {
        enable = true;
        # Source an init script generated by Nix below instead of spawning
        # Atuin to generate the same script in every interactive shell.
    ...
    programs.zsh = {
        ...
        initContent = lib.mkOrder 1500 ''
            ...
            source ${payRespectsZshInit}

            # Zoxide
            source ${zoxideZshInit}

            # Worktrunk init
            source "$HOME/.cache/shell-init/worktrunk.zsh"
            ...
            # uv-managed Python. Its default installation is validated during
            # Home Manager activation, so shells only need the path entry.
            ...
            # Static Atuin integration generated by Nix. This still creates a
            # fresh session UUID and installs the normal hooks in each shell.
            source ${atuinZshInit}
    ...
    home.activation.uvDefaultPython = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
        _uv_default_python="$HOME/.local/bin/python"
        _uv_python_dir="$(${lib.getExe pkgs.uv} python dir 2>/dev/null || true)"
        ...
        if [[ -z "$_uv_python_dir" || ! -x "$_uv_default_python" || "$_uv_default_python_target" != "$_uv_python_dir/"* ]]; then
            run ${lib.getExe pkgs.uv} python install --default --force
        fi
